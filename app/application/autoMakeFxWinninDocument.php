...
if($path = createFXWinningDocument($data))
{
    $data["path"] = $path;
    $data["s"] = 1;
    $data["r"] = "DATA_OK";
...
function createFXWinningDocument(array $data = null)
{
    $pdf = new FPDI();
    ...
    $path = GranCapital\FXWinning::getSourceTemplateOutput(TO_ROOT,time());

    $pdf->Output(GranCapital\FXWinning::getSourceTemplateOutput(TO_ROOT,time()), 'F');
